	...
	d3.csv("/csv/operations.csv", function(operations) {
		d3.csv("/csv/events.csv", function(events) {
			...
			function drawRects(operations, rooms, theGap, theTopPad, theSidePad, theBarHeight, theColorScale, w, h) {
				...
				var operationText = svg.append("g")
															.selectAll("text")
														 	.data(operations)
														 	.enter()
														 	.append("text")
															.text(function(d) {
																// return "OPNumber: " + d.OPNUMBER + ", StartTime: " + d.STARTTIME + ", EndTime: " + d.ENDTIME + ", Description: " + d.Description + ", Consultant: " + d.CONSULTANT + ", " + d.Patient_Name;
																return d.STARTTIME + " - " + d.ENDTIME
															})
															.attr("x", function(d) {
																var barStartTime = new Date(today.getFullYear() + "/" + (today.getMonth() + 1) + "/" + today.getDate() + " " + d.STARTTIME)
																var barEndTime = new Date(today.getFullYear() + "/" + (today.getMonth() + 1) + "/" + today.getDate() + " " + d.ENDTIME)
																return (x(barEndTime) - x(barStartTime)) / 2 + x(barStartTime) + theSidePad;
															})
															.attr("y", function(d, i) {
